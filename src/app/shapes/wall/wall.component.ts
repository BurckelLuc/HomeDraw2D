...
export class WallComponent extends ShapeComponent implements OnInit {
  ...
  constructor(private _viewContainerRef : ViewContainerRef, private toolService: ToolService, private shapesService: ShapesService) {
    super(_viewContainerRef);
    this.toolService.subscribe((x) => console.log(x))
  }
  ...
  onClick(event: MouseEvent): void {
    ...
    if (event.button == 0) {
      ...
      this.toolService.getTool().leftClick(closestPoint, this.shapesService, true)
    }
  }
  ...
  protected getCote(line: Line) {
    return Math.round(Math.sqrt(Math.abs(line.begin.x - line.end.x)**2 + Math.abs(line.end.y - line.begin.y)**2))
  }
  ...
  buildString() {
    ...
    stringBuilder = stringBuilder.concat(this.shape.lines.map(x => x.end).map(p => `L${p.x},${p.y}`));
